 ...
 * 反射工具类.
 *
 * <p>提供访问私有变量,获取泛型类型Class,提取集合中元素的属性,转换字符串到对象等Util函数.
 *
 * @author Li Teng Da
 */
@Slf4j
...
public class ReflectionUtil {
    ...
    public static Class<?> getSuperClass(final Object object) {
        Assert.notNull(object, "object不能为空");
        Class<?> curClass = object.getClass();
        if (curClass.getSuperclass() != Object.class) {
     ...
     * <p>如向上转型到Object仍无法找到, 返回null.
     */
    public static Class<?> getTopSuperClass(final Object object) {
        Assert.notNull(object, "object不能为空");
        Class<?> curClass = object.getClass();
        while (curClass.getSuperclass() != Object.class) {
            curClass = curClass.getSuperclass();
        }
        return curClass;
    }
     ...
     */
    public static String getSuperCaseClassName(Object object) {
        String result = null;
        if (object != null) {
            Class clazz = object.getClass();
            if (clazz != null) {
                String superClass = clazz.getSuperclass().getSimpleName();
                if (!"object".equalsIgnoreCase(superClass)) {
                    result = superClass.toUpperCase();
                } else {
                    result = clazz.getSimpleName().toUpperCase();
                }
            }
        }
        return result;
